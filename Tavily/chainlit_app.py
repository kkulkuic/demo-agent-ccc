"""
Chainlit app wrapping Tavilysample.py
...
# ─── Load Tavilysample as the agent backend ───
# Cached so InMemorySaver isn't recreated on re-import
_rn_module = None
...
def get_rn_module():
    ...
    if _rn_module is None:
        ...
        _spec = importlib.util.spec_from_file_location(
            "Tavilysample",
            os.path.join(os.path.dirname(__file__), "Tavilysample.py"))
        _rn_module = importlib.util.module_from_spec(_spec)
        sys.modules["Tavilysample"] = _rn_module
...
def make_tool_summary(tool_calls) -> str:
    ...
    for tc in tool_calls:
        ...
        if name == "places_search":
            ...
            parts.append(f'{icon} web_search: "{args.get("query", "")[:50]}"')
        elif name == "deep_scrape":
            parts.append(f'{icon} deep_scrape: "{args.get("url", "")[:50]}"')
        elif name == "web_fetch":
            parts.append(f'{icon} web_fetch: "{args.get("url", "")[:50]}"')
        elif name == "stop":
...
async def on_message(message: cl.Message):
        ...
        for node, data in event.items():
            ...
            if node in ("nav_agent", "research_agent"):
                ...
                for m in msgs:
                    ...
                    # Capture stop tool answer (nav agent only)
    ...
    # Stop button attached to a trailing message
